...
///
/// Число итераций считаем от ЁМКОСТИ кучи, а не от её размера в байтах: `usize`-бокс — 8 байт,
/// поэтому `2 * HEAP_SIZE / 8` штук вдвое перекрывают ёмкость (без переиспользования куча
/// кончилась бы на полпути). Так тест не замедляется линейно при росте `HEAP_SIZE`.
#[test_case]
fn many_boxes() {
    let count = 2 * heap::HEAP_SIZE / core::mem::size_of::<usize>();
    for i in 0..count {
        let x = Box::new(i);
        assert_eq!(*x, i);
    }
