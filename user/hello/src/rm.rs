//! `rm` (M7g3): удаляет файлы-аргументы через `unlink(2)`. Внешняя утилита `/bin`. Выход 0 при
//! успехе, 1 — если хоть один не удалился (нет файла / это каталог — для каталога `rmdir`).
...
extern "C" fn argv_main(sp: *const u64) -> ! {
    ...
    unsafe {
        ...
        let mut i = 1;
        while i < argc {
            if sc3(SYS_UNLINK, *argv.add(i) as u64, 0, 0) < 0 {
                write_str(b"rm: cannot remove file\n");
                code = 1;
            }
            i += 1;
        }
