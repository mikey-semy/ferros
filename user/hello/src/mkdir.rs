//! `mkdir` (M7f): создаёт каталоги-аргументы через `mkdir(2)`. Внешняя утилита `/bin`. Выход 0
//! при успехе, 1 — если хоть один не создался (нет родителя / уже существует / нет места).
...
extern "C" fn argv_main(sp: *const u64) -> ! {
    ...
    unsafe {
        if argc < 2 {
            write_str(b"usage: mkdir DIR...\n");
            sys_exit(1);
        }
        let mut code = 0u64;
        let mut i = 1;
        while i < argc {
            // mkdir(path, mode=0): mode ядро игнорирует.
            if sc3(SYS_MKDIR, *argv.add(i) as u64, 0, 0) < 0 {
                write_str(b"mkdir: cannot create directory\n");
                code = 1;
            }
            i += 1;
        }
