...
/* Массив строк окружения "ИМЯ=значение", завершён NULL. Заполняется crt0 из начального стека
 * (envp). У наших процессов окружение пока пустое (ядро передаёт пустой envp), но `getenv` работает
 * над тем, что есть (или что выставит программа). */
extern char **environ;

/* Возвращает значение переменной `name` (часть после '='), или NULL, если её нет. */
