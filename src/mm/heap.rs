...
/// Размер кучи — 1 МиБ. Поднят со 100 КиБ в M6f3: модель процессов держит на куче
/// управляющие структуры и ЯДРОВЫЙ стек каждого процесса (16 КиБ), а `fork`/`execve` ещё и
/// копируют адресные пространства и читают ELF с диска — нескольких процессов сразу 100 КиБ уже
/// не хватало.
