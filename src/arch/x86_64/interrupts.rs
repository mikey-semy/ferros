...
/// Входная частота PIT (Гц) и делитель по умолчанию: PIT тикает `PIT_INPUT_HZ / PIT_DIVISOR` раз
/// в секунду. Делитель не программируем (оставлен по умолчанию 65536 = «0»), отсюда ~18.2065 Гц.
const PIT_INPUT_HZ: u64 = 1_193_182;
const PIT_DIVISOR: u64 = 65_536;

/// Наносекунды с загрузки (uptime), посчитанные из счётчика тиков PIT (M9d). Это источник времени
/// для `clock_gettime`/`gettimeofday`. Точность — один тик (~54.9 мс): между тиками время «стоит».
/// Реального стенного времени (RTC) пока нет — отсчёт идёт от 0 на момент загрузки (см. HARDENING).
///
/// `ns = ticks * (PIT_DIVISOR / PIT_INPUT_HZ) * 1e9`; считаем через `u128`, чтобы не переполниться
/// и не потерять точность на целочисленном делении.
pub fn uptime_ns() -> u64 {
    let ns = ticks() as u128 * PIT_DIVISOR as u128 * 1_000_000_000u128 / PIT_INPUT_HZ as u128;
    ns as u64
}
