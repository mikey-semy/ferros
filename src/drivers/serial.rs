...
/// Пишет сырые байты в COM1 (для системного вызова `write` в stderr, M5b). В отличие от
/// VGA, serial — побайтово-точный канал (за ним настоящий UART, а не шрифт), поэтому
/// фильтрации непечатаемых байт тут НЕТ — намеренно. Замок держим под выключенными
/// прерываниями (то же правило против дедлока, что и в [`_print`]).
