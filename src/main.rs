...
fn kernel_main(boot_info: &'static BootInfo) -> ! {
    ...
    // M4e: вытесняющая многозадачность. Заводим планировщик потоков ядра и пару
    // фоновых потоков, которые НИЧЕГО не уступают — крутят `loop {}` и печатают
    // «сердцебиение». Включаем вытеснение по таймеру: теперь таймер сам переключает
    // потоки, и одно ядро честно делят фоновые потоки + executor (он гоняет
    // async-задачи на «нулевом» потоке). Зависший в цикле поток больше не захватит CPU.
    thread::init();
