...
impl Executor {
    ...
    pub fn run(&mut self) -> ! {
        loop {
            ...
            // Освобождаем ресурсы завершённых процессов (M6e3): «нулевой» поток (на котором
            // крутится экзекьютор) — безопасный контекст для этого (АП ядра, IF=1, не на
            // мёртвом стеке). No-op, если завершённых нет.
            super::thread::reap();
            self.sleep_if_idle();
        }
    }
